defmodule GameSiteWeb.RockPaperScissorsLive.Component do
  ...
  def instructions(assigns) do
    ...
    <div id="help-bubble" phx-hook="HelpBubble" class="relative inline-flex justify-center">
      ...
      <div
        data-help-panel
        class="absolute right-0 top-full z-50 mt-2 w-72 max-w-[calc(100vw-2rem)] rounded-xl border border-zinc-200 bg-white p-3 text-xs text-zinc-700 shadow-lg sm:text-sm dark:bg-zinc-900 dark:text-zinc-300 dark:border-zinc-800"
      >
  ...
  def wager(assigns) do
    ...
    <div class="grid grid-cols-1 gap-y-2 max-w-md mx-auto">
      <label for="wager_input" class="block text-sm font-medium text-zinc-700 mb-1 dark:text-zinc-300">
        Wager
      </label>
      <input
        type="number"
        ...
        max={@score}
        step="1"
        class="w-full rounded-md border-zinc-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:border-zinc-700"
      />
      <button
        type="button"
